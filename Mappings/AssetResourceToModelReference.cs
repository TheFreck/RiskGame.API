...
namespace RiskGame.API.Mappings
{
    public class AssetResourceToModelReference : Profile
    {
        ...
        {
            CreateMap<AssetResource, ModelReference >().ForMember(member => member.Id, config => config.MapFrom(og => og.AssetId));
        }
    }
    public class ModelReferenceToAssetResource : Profile
    {
        ...
        {
            CreateMap<ModelReference , AssetResource>().ForMember(member => member.AssetId, config => config.MapFrom(og => og.Id));
        }
    }
}
